...
public class MoveCamera : MonoBehaviour
{
    ...
    [Tooltip("Fallback distance if no camera distance is available (cm).")]
    ...
    void Update()
    {
        ...
        float distanceCm = UnityPythonConnector.Instance.distance > 0f ? UnityPythonConnector.Instance.distance : SittingDistanceCm;
        ...
        float headXcm = (0.5f - UnityPythonConnector.Instance.receivedValue.x) * 2f * halfTanH * distanceCm;
        float headYcm = (0.5f - UnityPythonConnector.Instance.receivedValue.y) * 2f * halfTanV * distanceCm;

        // Target offset in scene units
        Vector3 target = new Vector3(
            headXcm * worldScaleH * Sensitivity,
            headYcm * worldScaleV * Sensitivity,
            distanceCm * worldScaleH
        );
